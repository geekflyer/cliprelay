...
- After every major code change (new feature, bug fix, refactor), restart both apps so the user can immediately verify the fix:
  ...
  - **Android**: Install the new APK (`adb install -r dist/cliprelay-debug.apk`), force-stop the app (`adb shell am force-stop org.cliprelay`), and relaunch (`adb shell am start -n org.cliprelay/.ui.MainActivity`)
    ...
    - **Caution**: `settings put` with the already-set value is a silent no-op — dumpsys can show the service as "Bound" while it processes zero events (verify with a screen tap + `adb logcat -s ClipboardA11y` on a debug build). To force a real rebind, cycle: `settings put secure enabled_accessibility_services none`, then set the real value.
